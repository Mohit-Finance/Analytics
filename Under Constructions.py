...
def one_time(expiry_names):
    ...
    # ce_ltp_atm_1 = win2_plot00.plot([], pen=mkPen('g', width=3), name='CE OTM LTP 1')
    # pe_ltp_atm_1 = win2_plot00.plot([], pen=mkPen('y', width=3), name='PE OTM LTP 1')
    ...
    # ce_obv_atm_1 = pg.PlotDataItem(pen=mkPen(color='g', width=3, style=QtCore.Qt.DotLine), name='CE OTM LTP 1') # Create PlotDataItems to plot
    # win2_plot00_viewbox.addItem(ce_obv_atm_1) #Add PlotDataItems to the ViewBox

    # #Same Here - Repeat the Process
    # pe_obv_atm_1 = pg.PlotDataItem(pen=mkPen(color='y', width=3, style=QtCore.Qt.DotLine), name='PE OTM LTP 1')
    # win2_plot00_viewbox.addItem(pe_obv_atm_1)
    ...
    # win2_plot00.addLegend()
    # win2_plot00.legend.addItem(ce_obv_atm_1, 'CE OTM LTP 1')
    # win2_plot00.legend.addItem(pe_obv_atm_1, 'PE OTM LTP 1')
...
def chain(instrument_key,expiry_date,counter):
        ...
        # ce_df.to_excel('ce_df.xlsx')
        # pe_df.to_excel('pe_df.xlsx')
        ...
        if initialize==1:
            ...
            # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}'] = {'ce_atm+1_strike': [], 'ce_atm+2_strike': [], 'ce_atm+3_strike': [], 'ce_atm+1_ltp': [], 'ce_atm+2_ltp': [], 'ce_atm+3_ltp' : [], 'ce_atm+1_vol' : [], 'ce_atm+2_vol':[], 'ce_atm+3_vol':[], 'pe_atm+1_strike': [], 'pe_atm+2_strike': [], 'pe_atm+3_strike': [], 'pe_atm+1_ltp' : [], 'pe_atm+2_ltp' : [], 'pe_atm+3_ltp' : [], 'pe_atm+1_vol':[], 'pe_atm+2_vol':[], 'pe_atm+3_vol':[]}
        ...
        # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}']['ce_atm+1_strike'].append(int(ce_df.iloc[1,10]))
        # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}']['ce_atm+2_strike'].append(int(ce_df.iloc[2,10]))
        # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}']['ce_atm+1_ltp'].append(float(ce_df.iloc[1,9]))
        # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}']['ce_atm+2_ltp'].append(float(ce_df.iloc[2,9]))
        # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}']['ce_atm+1_vol'].append(int(ce_df.iloc[1,8]))
        # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}']['ce_atm+2_vol'].append(int(ce_df.iloc[2,8]))

        # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}']['pe_atm+1_strike'].append(int(pe_df.iloc[-2,10]))
        # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}']['pe_atm+2_strike'].append(int(pe_df.iloc[-3,10]))
        # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}']['pe_atm+1_ltp'].append(float(pe_df.iloc[-2,11]))
        # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}']['pe_atm+2_ltp'].append(float(pe_df.iloc[-3,11]))
        # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}']['pe_atm+1_vol'].append(int(pe_df.iloc[-2,12]))
        # past_data_obv[f'Today : {t_date} | {instrument_key} | Expiry : {expiry_date}']['pe_atm+2_vol'].append(int(pe_df.iloc[-3,12]))
...
def main_obv(setx):
        ...
        df=setx.copy()
        df['sstrike'] = df.iloc[:,0] != df.iloc[:,0].shift(1)
        df['group'] = df['sstrike'].cumsum()

        df['volume'] = np.where(df['sstrike']==True, df.iloc[:,2], df.iloc[:,1] - df.iloc[:,1].shift(1))
        df['volume'] = np.where(df['volume']>0, df.iloc[:,2], np.where(df['volume']<0, - df.iloc[:,2],0))
        df['obv'] = df.groupby('group')['volume'].cumsum()

        result = pd.DataFrame({'ltp': df.iloc[:, 1], 'obv': df['obv']})

        return result

def obv_new(df):
    df_exp0 = pd.DataFrame({'strike_ce_exp0_0' : pd.Series(df.iloc[0,0]), 'strike_ce_exp0_1': pd.Series(df.iloc[1,0]), 'ltp_ce_exp0_0':pd.Series(df.iloc[3,0]), 'ltp_ce_exp0_1':pd.Series(df.iloc[4,0]), 'vol_ce_exp0_0':pd.Series(df.iloc[6,0]), 'vol_ce_exp0_1':pd.Series(df.iloc[7,0]), 'strike_pe_exp0_0' : pd.Series(df.iloc[9,0]), 'strike_pe_exp0_1': pd.Series(df.iloc[10,0]), 'ltp_pe_exp0_0':pd.Series(df.iloc[12,0]), 'ltp_pe_exp0_1':pd.Series(df.iloc[13,0]), 'vol_pe_exp0_0':pd.Series(df.iloc[15,0]), 'vol_pe_exp0_1':pd.Series(df.iloc[16,0])})
    df_exp1 = pd.DataFrame({'strike_ce_exp1_0' : pd.Series(df.iloc[0,3]), 'strike_ce_exp1_1': pd.Series(df.iloc[1,3]), 'ltp_ce_exp1_0':pd.Series(df.iloc[3,3]), 'ltp_ce_exp1_1':pd.Series(df.iloc[4,3]), 'vol_ce_exp1_0':pd.Series(df.iloc[6,3]), 'vol_ce_exp1_1':pd.Series(df.iloc[7,3]), 'strike_pe_exp1_0' : pd.Series(df.iloc[9,3]), 'strike_pe_exp1_1': pd.Series(df.iloc[10,3]), 'ltp_pe_exp1_0':pd.Series(df.iloc[12,3]), 'ltp_pe_exp1_1':pd.Series(df.iloc[13,3]), 'vol_pe_exp1_0':pd.Series(df.iloc[15,3]), 'vol_pe_exp1_1':pd.Series(df.iloc[16,3])})
    df_exp2 = pd.DataFrame({'strike_ce_exp2_0' : pd.Series(df.iloc[0,4]), 'strike_ce_exp2_1': pd.Series(df.iloc[1,4]), 'ltp_ce_exp2_0':pd.Series(df.iloc[3,4]), 'ltp_ce_exp2_1':pd.Series(df.iloc[4,4]), 'vol_ce_exp2_0':pd.Series(df.iloc[6,4]), 'vol_ce_exp2_1':pd.Series(df.iloc[7,4]), 'strike_pe_exp2_0' : pd.Series(df.iloc[9,4]), 'strike_pe_exp2_1': pd.Series(df.iloc[10,4]), 'ltp_pe_exp2_0':pd.Series(df.iloc[12,4]), 'ltp_pe_exp2_1':pd.Series(df.iloc[13,4]), 'vol_pe_exp2_0':pd.Series(df.iloc[15,4]), 'vol_pe_exp2_1':pd.Series(df.iloc[16,4])})

    df = pd.concat([df_exp0, df_exp1, df_exp2], axis=1)

    sett0 = df[['strike_ce_exp0_0', 'ltp_ce_exp0_0', 'vol_ce_exp0_0']]
    sett1 = df[['strike_ce_exp0_1', 'ltp_ce_exp0_1', 'vol_ce_exp0_1']]
    sett2 = df[['strike_pe_exp0_0', 'ltp_pe_exp0_0', 'vol_pe_exp0_0']]
    sett3 = df[['strike_pe_exp0_1', 'ltp_pe_exp0_1', 'vol_pe_exp0_1']]

    sett4 = df[['strike_ce_exp1_0', 'ltp_ce_exp1_0', 'vol_ce_exp1_0']]
    sett5 = df[['strike_ce_exp1_1', 'ltp_ce_exp1_1', 'vol_ce_exp1_1']]
    sett6 = df[['strike_pe_exp1_0', 'ltp_pe_exp1_0', 'vol_pe_exp1_0']]
    sett7 = df[['strike_pe_exp1_1', 'ltp_pe_exp1_1', 'vol_pe_exp1_1']]

    sett8 = df[['strike_ce_exp2_0', 'ltp_ce_exp2_0', 'vol_ce_exp2_0']]
    sett9 = df[['strike_ce_exp2_1', 'ltp_ce_exp2_1', 'vol_ce_exp2_1']]
    sett10 = df[['strike_pe_exp2_0', 'ltp_pe_exp2_0', 'vol_pe_exp2_0']]
    sett11 = df[['strike_pe_exp2_1', 'ltp_pe_exp2_1', 'vol_pe_exp2_1']]

    list_obv_df = []
    setts = [sett0, sett1, sett2, sett3, sett4, sett5, sett6, sett7, sett8, sett9, sett10, sett11]
    
    for sett in setts:
        dff = main_obv(sett)
        list_obv_df.append(dff)

    exp0 = pd.concat([list_obv_df[0], list_obv_df[1], list_obv_df[2], list_obv_df[3]], axis=1)
    exp1 = pd.concat([list_obv_df[4], list_obv_df[5], list_obv_df[6], list_obv_df[7]], axis=1)
    exp2 = pd.concat([list_obv_df[8], list_obv_df[9], list_obv_df[10], list_obv_df[11]], axis=1)
...
def call():
    ...
    # with open(f'Credentials/Data/{tdate}_past_data_obv.json', 'w') as file_write:
    #     json.dump(past_data_obv, file_write)

    df = pd.DataFrame(past_data)
    # df_obv = pd.DataFrame(past_data_obv)
    ...
    dfpp = vwap(dfp)    
    obv_df = obv(dfp)
    # nifty_ltp_obv, bnf_ltp_obv, sensex_ltp_obv = obv_new(df_obv)
